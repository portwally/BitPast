...
// MARK: - MSX Disk Writer
// MSX disk image writer supporting DSK format (MSX-DOS FAT12)

class MSXDiskWriter {
    static let shared = MSXDiskWriter()
    private init() {}

    // MARK: - MSX Disk Constants

    private let sectorSize = 512
    private let sectorsPerCluster = 2
    private let reservedSectors = 1
    private let numberOfFATs = 2
    private let rootDirEntries = 112
    ...
    func createDiskImage(at url: URL, volumeName: String, size: DiskSize, files: [URL]) -> Bool {
        ...
        // Boot sector with MSX-DOS signature
        ...
        do {
            ...
            print("MSXDiskWriter: Created DSK at \(url.path)")
            return true
        } catch {
            print("MSXDiskWriter: Failed to write DSK: \(error)")
            return false
        }
    }
    ...
    private func initializeBootSector(in data: inout Data, volumeName: String,
        ...
        data[17] = UInt8(rootDirEntries & 0xFF)
        data[18] = UInt8((rootDirEntries >> 8) & 0xFF)
    ...
    private func diskGeometry(for size: DiskSize) -> DiskGeometry {
        switch size {
        case .kb360:
            return DiskGeometry(totalSectors: 720, sectorsPerTrack: 9, heads: 2, mediaDescriptor: 0xF9)
        case .kb720:
            return DiskGeometry(totalSectors: 1440, sectorsPerTrack: 9, heads: 2, mediaDescriptor: 0xF9)
        default:
            return DiskGeometry(totalSectors: 1440, sectorsPerTrack: 9, heads: 2, mediaDescriptor: 0xF9)
        }
    }
